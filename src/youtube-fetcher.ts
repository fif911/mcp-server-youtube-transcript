      ...
      for (const track of tracks) {
        if (track.languageCode) {
          availableLanguages.push({
            ...
            baseUrl: track.baseUrl || undefined
          });
        }
      }
    } catch (e) {
...
/**
 * Parses timedtext response (handles both XML and JSON formats).
 * XML format: <p t="1200" d="2160">text</p>
 * JSON format: { events: [{ tStartMs, dDurationMs, segs: [{utf8}] }] }
 */
function parseTimedTextResponse(body: string): TranscriptLine[] | null {
  ...
  if (body.trimStart().startsWith('{')) {
    try {
      ...
      // Not valid JSON, fall through to XML
    }
  }

  // Parse XML format: <p t="ms" d="ms">text</p>
...
/**
 * Fetches transcript via timedtext URL.
 * Returns TranscriptLine[] or null if unavailable.
 */
async function fetchTimedText(baseUrl: string, userAgent: string = WEB_USER_AGENT): Promise<TranscriptLine[] | null> {
  try {
    const parsed = new URL(baseUrl);
    console.error(`[youtube-fetcher] Fetching timedtext from: ${parsed.hostname}${parsed.pathname}?...`);
    ...
    const lines = parseTimedTextResponse(response);
    console.error(`[youtube-fetcher] Timedtext: got ${lines?.length || 0} lines`);
    return lines;
  } catch (err) {
    console.error(`[youtube-fetcher] Timedtext fetch failed: ${(err as Error).message}`);
    return null;
  }
}

/**
 * Fetches caption track URLs via the /player endpoint (ANDROID client).
 * This is the most reliable method — YouTube's player API returns fresh signed URLs.
 */
async function fetchPlayerCaptions(videoId: string, visitorData: string): Promise<{ languageCode: string; baseUrl: string; kind?: string }[] | null> {
  try {
    ...
    console.error(`[youtube-fetcher] Fetching /player for caption URLs...`);
    const response = await httpsRequest({
      hostname: 'www.youtube.com',
    ...
    if (!Array.isArray(tracks) || tracks.length === 0) {
      console.error(`[youtube-fetcher] /player: no caption tracks found`);
      return null;
    }

    console.error(`[youtube-fetcher] /player: found ${tracks.length} caption tracks`);
    ...
    console.error(`[youtube-fetcher] /player fetch failed: ${(err as Error).message}`);
    return null;
  }
}
...
export async function getSubtitles(options: {
  ...
  const { visitorData, clientVersion, availableLanguages, chapters, adChapters, metadata, isLive } = await getPageData(videoID);
  ...
  // Strategy 1: Fetch caption URLs via /player endpoint and download timedtext directly
  const playerTracks = await fetchPlayerCaptions(videoID, visitorData);
  if (playerTracks && playerTracks.length > 0) {
    const playerTrack = playerTracks.find(t => t.languageCode === targetLang) || playerTracks[0];
    if (playerTrack?.baseUrl) {
      console.error(`[youtube-fetcher] Trying /player timedtext for '${playerTrack.languageCode}'...`);
      const timedTextLines = await fetchTimedText(
        playerTrack.baseUrl,
        'com.google.android.youtube/20.10.38 (Linux; U; Android 14) gzip'
      );
      if (timedTextLines && timedTextLines.length > 0) {
        console.error(`[youtube-fetcher] /player timedtext succeeded: ${timedTextLines.length} lines`);
        return {
          lines: timedTextLines,
          requestedLang: lang,
          actualLang: playerTrack.languageCode,
          availableLanguages,
          chapters,
          adChapters,
          metadata,
      ...
      console.error(`[youtube-fetcher] /player timedtext returned no lines, falling back to get_transcript API`);
    }
  }

  // Strategy 2: Fall back to get_transcript API with multiple clients
  ...
  // WEB client — the correct client for get_transcript endpoint
  ...
  const payloads = [
    { name: 'WEB', payload: webPayload, userAgent: WEB_USER_AGENT },
